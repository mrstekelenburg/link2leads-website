**Correctie 25 september 2026: Engels vertaalde grote delen van de homepage niet mee**
- Oorzaak: de versie van het databestand met vertalingen stond in `l2l-i18n.js` zelf (`?v=31`), terwijl dat script op de pagina's al sinds 22 september onder `?v=32` staat en `/assets/` zeven dagen gecachet wordt. Browsers en de CDN hielden dus het oude script vast, dat naar het oude databestand (`?v=30`) wees. Alles wat op 25 september is toegevoegd (vergelijkingstabel, mini-reviews, prijsintro, contractvolume) had wel een vertaling, maar die werd niet opgehaald.
- Opgelost: `l2l-i18n.js` leest voortaan zijn eigen `?v=` uit de scripttag en haalt het databestand met datzelfde nummer op. Er is nog maar een nummer om op te hogen, in de HTML. Alle 25 pagina's staan nu op `l2l-i18n.js?v=33`; de pagina's die nog op `?v=28` of `?v=31` stonden zijn gelijkgetrokken.
- Alle 25 pagina's nagelopen met een script dat precies doet wat de vertaalengine doet (tekstnodes, placeholders, titles, alt, aria-labels, title en meta's). Wat nog ontbrak en nu een EN-sleutel heeft: de vijf korte reviewquotes op de homepage (de aanhalingstekens staan in dezelfde tekstnode als de quote, dus de sleutel zonder aanhalingstekens matchte niet), de losse voegwoorden "en" en "of" tussen links, "wk 1-2" op de casegrafiek, de WhatsApp-title op de zwevende knop, en de meta description en og:title van de vijf nieuwe pagina's van 22 september en van `/leads`.
- Teksten die scripts zelf in de pagina zetten hadden ook geen vertaling: foutmeldingen en bevestigingen van de formulieren (marktscan, contact, leads, gratis sample, boeking), knopteksten tijdens het versturen, de samenvatting en de afrekennoot in de leadconfigurator op `/leads`, en het samengestelde pakket op `/book`. Waar een bedrag of aantal in de zin staat, staan de vaste stukken nu in eigen spans zodat ze los vertaald worden (38 sleutels).
- Gecontroleerd in een echte browser met `?lang=en` op homepage, `/leads`, `/book`, `/klant`, `/cases`, een case en een kennisartikel: geen Nederlandse tekst meer, alleen namen, plaatsen en merknamen.

**Aanvulling 25 september 2026: vergelijkingssectie en reviews door de homepage heen**
- Nieuwe sectie `#vergelijk` direct onder de prijssectie (na de accordeons, voor koopsignalen): "Vergelijkbaar met andere bureaus?" met een tabel van 17 rijen. Per rij: waar je op let (met korte uitleg), wat Link2Leads doet (blauwe kolom, groen vinkje) en wat andere bureaus meestal doen (rood kruis = meestal niet, geel plusminus = soms of tegen meerprijs). Rijen: verzenddagen, volume op het instappakket, volume in je contract, eigen pagina per prospect, persoonlijke openingszin, koopsignalen, segmentatie vooraf, opvolging kiezen (Full Service), LinkedIn erbij, setters en closers via Link2Talent, reacties handmatig filteren, garantie, geen portaal, geen setupfee, live binnen 14 dagen, wat van jou blijft, alleen de leadlijst. Geen concurrent bij naam genoemd. Onder de tabel een regel om ontbrekende vergelijkingspunten te mailen, plus de fitcheck-knop. Op mobiel wordt elke rij een kaart met twee blokken (Link2Leads en Andere bureaus), zonder horizontaal scrollen.
- Nieuwe component `.rv-mini`: een compacte reviewkaart (avatar, vijf sterren, 5,0, Google-logo, korte quote uit de volledige review, naam en bedrijf, link naar `#reviews`). Vijf keer geplaatst: onder "Zo werkt het" (Micah, MIJU-marketing), tussen de pakketkaarten en de samensteller (Dick, Remorec), onder de vergelijkingstabel (Arnold, Rollecate), onder koopsignalen (Rafael, CrudenVision) en onder de FAQ (Marit, initiaal als avatar omdat er geen logo of foto is). De avatar is het bedrijfslogo; er zijn geen foto's van de reviewers, die kunnen later per kaart in `src` gezet worden.
- Volumegarantie overal gelijkgetrokken: het maandvolume staat op elk pakket in het contract (3.000 / 7.500 / 15.000 / 30.000), niet alleen op Pro met "naar rato". Aangepast in de vergelijkingstabel, het accordeon "Garantie en contract", de voetnoot onder de weekendtabel, het FAQ-schema en de vier Offer-beschrijvingen in schema.org, `kennis/leadgeneratie-bureau-kiezen.html`, `llms.txt`, `llms-full.txt` en de NL/EN-sleutels in `l2l-i18n-data.js`.
- Alle formuleringen over uitval en inhalen ("valt er een dag uit, dan halen we dat in", "bij een technische hapering wordt het volume ingehaald", "inhaalregeling", "inhaalplicht") zijn weggehaald, omdat ze suggereren dat uitval voorkomt. Vervangen door "elke maand zie je het werkelijke aantal verzonden mails" of gewoon weggelaten. Op de homepage, in `cold-email-bureaus-nederland.html`, twee kennisartikelen, `llms.txt`, `llms-full.txt` en de NL/EN-teksten in `l2l-i18n-data.js`. De checklistvraag "wat er gebeurt als een dag uitvalt" in het artikel over een bureau kiezen staat er nog, dat is een vraag aan bureaus in het algemeen.
- Intro van de prijssectie eindigt nu met een link "Vergelijk ons met andere bureaus →" naar `#vergelijk`.
- Vergelijkingstabel teruggebracht van 17 naar 10 rijen (volume, personalisatie per prospect, koopsignalen en segmentatie, reacties filteren, opvolging inclusief Link2Talent, LinkedIn erbij, garantie, setup, wat van jou blijft, alleen de leadlijst). "Geen portaal" eruit. Vinkje en tekst staan nu in een flexblok naast elkaar, zodat de tekst netjes uitlijnt bij meerdere regels.
- "Staat de setup niet op tijd door iets aan onze kant, dan schuift je facturatie mee" en de variant "Loopt het uit..." overal weggehaald (setupgarantie op de homepage, FAQ op `leadgeneratie-amsterdam.html`, `llms-full.txt`, NL/EN in de vertaaldata). Het komt niet voor, dus we benoemen het niet.
- SEO en llms: de vergelijkingstabel staat als sectie in `llms.txt` (met verwijzing naar #vergelijk), de datum in `llms-full.txt` is bijgewerkt, `sitemap.xml` heeft lastmod 25 september voor de vijf gewijzigde pagina's, en de review van Rafael (CrudenVision) is toegevoegd aan de Review-lijst in schema.org op de homepage, zodat alle vijf getoonde reviewers ook in de gestructureerde data staan.
- Mobiel (tot 760px) heeft de vergelijkingstabel een eigen compacte vorm: per punt de titel, daaronder een korte regel voor Link2Leads (vinkje, wit, vet) en een korte regel voor andere bureaus (kruis of plusminus, grijs). De uitleg onder de titel en de labels "Link2Leads / Andere bureaus" staan op mobiel niet; de korte teksten staan als aparte `span.t-m` in dezelfde cel en zijn op desktop verborgen. Daarna (13:10) opnieuw opgezet als losse kaarten: per punt een kaart met icoon en blauwe titel, daarin een blauw blok "Bij ons" met blauwe linkerrand en een grijs blok "Andere bureaus" met grijze linkerrand, zodat de twee kanten duidelijk gescheiden zijn. De legenda is op mobiel verborgen, de voettekst met de fitcheck-knop is een eigen kaart.
- Iconen (inline SVG, blauw vlakje in de stijl van de site) toegevoegd op vier plekken: de drie koopsignaalkaarten, de drie add-ons in de samensteller, de tien rijtitels in de vergelijkingstabel (desktop boven de titel, mobiel ernaast) en de vier accordeons onder de pakketten. De rijtitels in de vergelijkingstabel staan in het accentblauw; Link2Talent in die tabel volgt de merkopmaak (2 blauw en schuin).
- Menu (desktop en mobiel) heeft er een link "Vergelijk" bij, tussen Pakketten en Leads kopen.
- Engelse vertalingen van alle nieuwe en gewijzigde teksten (vergelijkingstabel, reviewkaarten, accordeontitels, koopsignaaltitels, prijsintro, volume in contract) toegevoegd aan `l2l-i18n-data.js` (101 sleutels, blok onderaan), cachebuster naar `?v=31`. De mobiele labels "Bij ons / Andere bureaus" in de vergelijking zijn CSS-content en krijgen hun Engelse tekst via `html[lang=en]`. Gecontroleerd met `?lang=en`: geen Nederlandse tekst meer in de nieuwe secties, alleen namen en merknamen blijven staan.
