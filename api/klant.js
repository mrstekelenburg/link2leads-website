...
module.exports = async (req, res) => {
  ...
  const notifyEmail = process.env.NOTIFY_EMAIL || 'demi@link2leads.nl';
...
<div style="max-width:580px;margin:24px auto;background:#12141C;border-radius:14px;overflow:hidden;border:1px solid rgba(255,255,255,0.1)">
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"Link2Leads" <${process.env.MAIL_FROM || 'info@link2leads.nl'}>`,
      to: email,
    ...
    await transporter.sendMail({
      from: `"Link2Leads Onboarding" <${process.env.MAIL_FROM || 'info@link2leads.nl'}>`,
      to: notifyEmail,
